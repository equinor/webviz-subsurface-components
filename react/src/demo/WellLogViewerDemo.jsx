...
class WellLogViewerDemo extends Component {
    ...
    insertFileItem(value) {
        return (<span>
                <input type="radio" id={value}  checked={this.state.example == value}  onChange={this.onFileChange.bind(this)} />
    ...
    insertTemplateItem(name, value) {
        return (
        <option value={value}>{name? name: "Template "+(parseInt(value)+1).toString()}</option>
        );
        }
             ...
             <div>
                <div align="center">
                    Template:
                        ...
                        >
                            {templates.map((template, i) => this.insertTemplateItem(template.name, i))}
                        </select>
                    <span> &nbsp; &nbsp; &nbsp; </span>
                    Data file: {examples.map((example,i) => this.insertFileItem(i))}
                 </div>
                ...
                <div style={{ width: "20%", flex: "none" }}>
                    ...
                    <WellLogViewer welllog={this.getData(this.state.example)} template={JSON.parse(this.state.text)} />
                        </div>
               </div>
            </div>
        );
