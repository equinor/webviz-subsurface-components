...
export const ExpressionsTableComponent: React.FC<ExpressionsTableComponentProps> = (props: ExpressionsTableComponentProps) => {
    const { expressions } = props;
    const [activeExpression, setActiveExpression] = React.useState<ExpressionType | undefined>(undefined);
    ...
    const handleDeleteClick = (): void => {
        const newExpressionsList = expressions.filter((elm) => { return selectedExpressions.indexOf(elm) === -1 });
        props.onExpressionsChange(newExpressionsList);
        setSelectedExpressions([]);



        if (activeExpression !== undefined && !newExpressionsList.some(el => el.id === activeExpression.id)) {
    ...
    const handleNewClick = (): void => {
        ...
        const newExpression: ExpressionType = { name: newName, expression: "", id: uuidv4(), variableVectorMap: [] };
    ...
    const addNewExpressions = useCallback((newExpressions: ExpressionType[]): void => {
    ...
    return (
        <Paper className="ExpressionTableComponent">
            ...
            <Grid container item spacing={2} alignItems="flex-end">
                ...
                <Grid item><Button onClick={handleNewClick}><Icon key="new" name="add" />New</Button></Grid>
            </Grid>
        </Paper >
    );
};
