...
import { Paper, Table, TableContainer, TableRow, TableBody, TableCell } from '@material-ui/core';
...
export const VariablesTable: React.FC<VariablesTableProps> = (props: VariablesTableProps) => {
    const { vectorData } = props;

    const updatedProps = (vectorSelectorProps: any, index: number): void => {
        if (vectorSelectorProps.selectedTags.length !== 1) {
            return;
        }
        const newVariableVectorMap = cloneDeep(props.variableVectorMap);
        newVariableVectorMap[index].vectorName[0] = vectorSelectorProps.selectedTags[0];
                ...
                <TableBody>
                    {props.variableVectorMap.map((row, index) => {
                        return (
                            ...
                            >
                                <TableCell align="left" key={row.variableName}>{row.variableName}</TableCell>
                                <TableCell key={`cell_${row.variableName}`}>
                                    <VectorSelector
                                        id={"vector_selector_" + row.variableName}
                                        key={"vector_selector_" + row.variableName}
                                        delimiter=":"
                                        label=""
                                        selectedTags={row.vectorName}
                                        setProps={(props) => updatedProps(props, index)}
                                        numMetaNodes={0}
