...
vec4 calcColorValue(float color_value, float scale_type) {
    ...
    if (mapped_color_value > u_max_color_value || mapped_color_value < u_min_color_value) {
...
void main() {
    ...
    gl_FragColor = calcColorValue(color_value, u_scale_type);
}