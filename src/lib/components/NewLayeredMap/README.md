...
If added to the map through the _layers_ prop, these shape objects (except for the circleMarker) will be emited on click with the [clicked_shape listener](#listeners). It is possible for the python user to add extra fields, such as `"name"` or `"shapeID"` (the names are up to you) e.g. in order to simplify editing a single shape in a layer.
...
| **clicked_shape**  | [Shape object](#polyline) | JSON object containing the details of last shape clicked   |
